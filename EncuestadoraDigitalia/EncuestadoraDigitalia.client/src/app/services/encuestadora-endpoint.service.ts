...
@Injectable()
export class EncuestadoraEndpoint extends EndpointBase {
  ...
  get encuestasUrl() { return this.configurations.baseUrl + '/api/encuestadora/encuestas'; }
  ...
  getEncuestasEndpoint<T>(page?: number, pageSize?: number): Observable<T> {
    const endpointUrl = page && pageSize ? `${this.encuestasUrl}/${page}/${pageSize}` : this.encuestasUrl;

    return this.http.get<T>(endpointUrl, this.requestHeaders).pipe(
      catchError(error => {
        return this.handleError(error, () => this.getEncuestasEndpoint<T>(page, pageSize));
      }));
  }
  ...
  getDeleteEncuestaEndpoint<T>(encuestaId: number): Observable<T> {
    const endpointUrl = `${this.encuestasUrl}/${encuestaId}`;

    return this.http.delete<T>(endpointUrl, this.requestHeaders).pipe(
      catchError(error => {
        return this.handleError(error, () => this.getDeleteEncuestaEndpoint<T>(encuestaId));
      }));
  }
